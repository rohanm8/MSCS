...
#define MAX_RETRIES 3 // Maximum number of retries for failed downloads
/*
1. in void start_download_threads
	- changed data type of part_size from int to long
2. in int get_filsize
	- changed data type of file_size (int get_filesize) from int to long in every instance
3. changed
*/
...
void start_download_threads(char *url, int num_parts, long file_size, char *output_file);
...
int main(int arg_count, char *arg_val[]) {
	...
	long file_size = get_filesize(url);
	if (file_size == -1) {
